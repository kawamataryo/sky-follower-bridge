...
function IndexPopup() {
  ...
  const searchBskyUser = async (e: FormEvent) => {
    ...
    const [{url: currentUrl}] = await chrome.tabs.query({ active: true, currentWindow: true })
    ...
    const messageName = match(currentUrl)
      .with(P.when((url) =>  TARGET_URLS_REGEX.FOLLOW.test(url)), () => MESSAGE_NAMES.SEARCH_BSKY_USER_ON_FOLLOW_PAGE)
      .with(P.when((url) =>  TARGET_URLS_REGEX.BLOCK.test(url)), () => MESSAGE_NAMES.SEARCH_BSKY_USER_ON_BLOCK_PAGE)
      .with(P.when((url) =>  TARGET_URLS_REGEX.MUTE.test(url)), () => MESSAGE_NAMES.SEARCH_BSKY_USER_ON_MUTE_PAGE)
      .run()
    ...
    try {
      ...
      console.log("🚀 ~ file: popup.tsx:60 ~ searchBskyUser ~ res:", res)
      if(res.hasError) {
      ...
      <form onSubmit={searchBskyUser} className="mt-2">
        ...
        {isShowErrorMessage && (
          <div className="alert text-red-600 border border-red-600 p-2 rounded-md mt-2">
            <div>
              <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" stroke-linejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
